...
Releases are built by [cargo-dist](https://axodotdev.github.io/cargo-dist/) on
GitHub Actions and uploaded as a GitHub Release. **Nothing is built unless you
publish a release** — a pull request only runs the (fast) plan check.
...
grammars is the bulk of it, and the three targets build in parallel.

### What gets built

| Platform | Target | Artifacts |
| --- | --- | --- |
| Linux x86_64 | `x86_64-unknown-linux-gnu` | `.tar.gz`, `.deb`, `.rpm`, Arch `.pkg.tar.zst`, AppImage, `curl \| sh` installer |
| Windows x86_64 | `x86_64-pc-windows-msvc` | `.zip`, `.msi`, PowerShell installer |
| macOS Apple Silicon | `aarch64-apple-darwin` | `.tar.gz` |

All artifacts ship a `sha256` checksum, and a combined `sha256.sum` is attached
to the release. There is also a source tarball.
...
There are two workflows, and it is worth knowing why:

- **`.github/workflows/release.yml`** is generated by cargo-dist. Do not edit it
  by hand; it is checked against the config on every run. Build it with
  `dist generate --mode=ci`.
- **`.github/workflows/linux-packages.yml`** is hand written and hooked in from
  `global-artifacts-jobs` in `Cargo.toml`. It exists because cargo-dist has no
  `.deb`, `.rpm`, Arch or AppImage backend — its installer backends are `shell`,
  `powershell`, `npm`, `homebrew`, `msi` and `pkg`. It turns the Linux tarball
  dist produced into those four formats.

If you add a target, add it to `targets` in `Cargo.toml` and re-run
`dist generate --mode=ci` — but comment out `allow-dirty` first, and re-apply
the `release: published` trigger afterwards, because regeneration overwrites it.
