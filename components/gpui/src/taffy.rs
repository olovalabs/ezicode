...
impl ToTaffy<taffy::style::Style> for Style {
    fn to_taffy(&self, rem_size: Pixels, scale_factor: f32) -> taffy::style::Style {
        ...
        fn to_grid_repeat<T: taffy::style::CheapCloneStr>(
            ...
            // The literals are f32: `length`/`fr` take `Into<f32>`, and leaving
            // them untyped makes the compiler fall back to f64, which does not
            // implement `From<f64>` for f32.
            unit.map(|count| {
                vec![repeat(count, vec![minmax(length(0.0_f32), fr(1.0_f32))])]
            })
            .unwrap_or_default()
        }
