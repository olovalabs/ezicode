...
/// The platform's real text system, which shapes and rasterizes text.
///
/// Tests normally get [`NoopTextSystem`], in which nothing is ever drawn. A
/// headless test context needs this one, or every string would render as nothing.
#[cfg(all(
    any(test, feature = "test-support"),
    any(target_os = "linux", target_os = "freebsd"),
    any(feature = "x11", feature = "wayland")
))]
pub(crate) fn platform_text_system() -> Result<Arc<dyn PlatformTextSystem>> {
    Ok(Arc::new(crate::CosmicTextSystem::new()))
}
...
pub(crate) fn platform_text_system() -> Result<Arc<dyn PlatformTextSystem>> {
    Ok(Arc::new(crate::MacTextSystem::new()))
}

/// On Windows the text system is a DirectWrite object built alongside the
/// platform's display, so a headless context cannot have one.
#[cfg(all(
    any(test, feature = "test-support"),
    not(any(
        all(
...
/// Renders a [`Scene`] without a window, a surface, or a display server.
///
/// The test platform owns one of these and routes every window `draw` through
/// it, which is the only way a test can observe pixels. It stays crate-private
/// because [`Scene`] is: callers reach it through
/// [`crate::VisualTestContext::draw`] instead.
///
/// The renderer and the window must be created together and share one atlas:
/// window painting resolves sprites through
/// [`PlatformHeadlessRenderer::sprite_atlas`], so a window paired with a
/// different atlas samples the wrong tiles.
#[cfg(any(test, feature = "test-support"))]
pub(crate) trait PlatformHeadlessRenderer {
    /// Draws `scene` at `size` and throws the pixels away.
    ///
    /// The target is reused between calls of the same size, so this is the cheap
    /// path: it measures encoding and submission, not readback.
    fn render_scene(&mut self, scene: &Scene, size: Size<DevicePixels>) -> Result<()>;

    /// Draws `scene` at `size` and reads the result back as RGBA pixels.
    ...
    /// The sprite atlas this renderer samples from.
    fn sprite_atlas(&self) -> Arc<dyn PlatformAtlas>;
}
