use crate::{
    ...
    WindowHandle, WindowOptions, point, px,
};
...
impl TestAppContext {
    ...
    /// Creates a test context that really renders, with no window and no display
    /// server: every window opened on it draws through a headless GPU renderer,
    /// and [`VisualTestContext::draw`] returns the resulting pixels.
    ///
    /// This is what you want for snapshot tests, pixel assertions, and
    /// benchmarks of your own views. Errors if no GPU can be acquired, rather
    /// than silently rendering nothing.
    ///
    /// Software adapters are rejected. Use
    /// [`Self::with_headless_renderer_allowing_software`] on a machine that only
    /// has a software Vulkan driver.
    pub fn with_headless_renderer() -> anyhow::Result<Self> {
        Self::with_headless_renderer_allowing_software(false)
    }

    /// Like [`Self::with_headless_renderer`], but accepts a software adapter.
    ///
    /// Frames then measure (and look like) CPU rasterization, which is fine for
    /// functional tests but not for anything you intend to benchmark.
    pub fn with_headless_renderer_allowing_software(
        ...
        let platform_text_system = crate::platform::platform_text_system()?;
        let platform = TestPlatform::new_with_headless_renderer(
            background_executor.clone(),
            foreground_executor.clone(),
            Arc::clone(&platform_text_system),
            allow_software_adapter,
        )?;
        let text_system = Arc::new(TextSystem::new(platform_text_system));

        Ok(Self {
            app: App::new_app(platform.clone(), Arc::new(()), {
    ...
    /// Like [`Self::add_window_view`], but with an explicit window size.
    ///
    /// Worth using whenever the window is actually rendered: the default
    /// maximized window is 1920x1080, which a headless renderer has to
    /// rasterize and read back at twice that in each direction.
    pub fn add_sized_window_view<F, V>(
        &mut self,
...
impl VisualTestContext {
    ...
    /// Renders the window's current state and returns the pixels.
    ///
    /// The window is painted and presented exactly as it would be on screen, and
    /// the frame is read back, so this is a real render: text is shaped and
    /// rasterized through the sprite atlas. The image is in device pixels, so its
    /// dimensions are the window size times the window's scale factor.
    ///
    /// Only contexts built by [`TestAppContext::with_headless_renderer`] have a
    /// renderer; on a plain [`TestAppContext`] this fails rather than quietly
    /// returning a blank image.
    pub fn draw_frame(&mut self) -> anyhow::Result<image::RgbaImage> {
        let window = self.cx.test_window(self.window);
        window.capture_next_frame();

        self.paint_and_present()?;

        window.take_capture_result().ok_or_else(|| {
            anyhow!(
                "the window was not rendered: this test context has no headless renderer, \
                 build it with TestAppContext::with_headless_renderer()"
            )
        })?
    }

    /// Renders the window without reading the pixels back.
    ///
    /// This is the path to benchmark: the frame is encoded, submitted and awaited,
    /// but nothing is copied to the CPU, so a loop of these measures the work a
    /// frame does rather than the readback. Note that it still includes text
    /// shaping and sprite-atlas uploads, and that frames do not overlap, so it
    /// says nothing about how long the GPU takes to finish.
    ///
    /// Requires a context built by [`TestAppContext::with_headless_renderer`];
    /// on a plain [`TestAppContext`] this fails rather than quietly doing nothing.
    pub fn render(&mut self) -> anyhow::Result<()> {
        self.paint_and_present()
    }

    fn paint_and_present(&mut self) -> anyhow::Result<()> {
        self.cx.update_window(self.window, |_, window, cx| {
            window.draw(cx).clear();
            // A real platform presents from its frame callback; here we are
            // already on the thread that owns the window, so drive it directly.
            window.present();
        })
    }
