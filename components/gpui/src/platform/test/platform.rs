use crate::{
    ...
    DummyKeyboardMapper, ForegroundExecutor, Keymap, NoopTextSystem, Platform,
    PlatformDisplay, PlatformHeadlessRenderer, PlatformKeyboardLayout, PlatformKeyboardMapper,
    PlatformTextSystem, PromptButton, ScreenCaptureFrame, ScreenCaptureSource, ScreenCaptureStream,
    SourceMetadata, Task, TestAtlas, TestDisplay, TestWindow, WindowAppearance, WindowParams, size,
};
use anyhow::Result;
...
/// The headless renderer for this platform, if it has one.
///
/// Every platform GPUI supports with the blade backend can render off-screen.
#[cfg(any(
    all(
...
fn new_headless_renderer(
    allow_software_adapter: bool,
) -> anyhow::Result<Box<dyn PlatformHeadlessRenderer>> {
    crate::platform::blade::BladeHeadlessRenderer::new(allow_software_adapter)
...
fn new_headless_renderer(
    _allow_software_adapter: bool,
) -> anyhow::Result<Box<dyn PlatformHeadlessRenderer>> {
    anyhow::bail!("headless rendering is not supported on this platform")
}
...
pub(crate) struct TestPlatform {
    ...
    /// Builds the renderer behind every test window, if this context renders at
    /// all. Windows take their sprite atlas from it, so a window and its
    /// renderer are always created together.
    headless_renderer_factory:
        Option<Box<dyn Fn() -> anyhow::Result<Box<dyn PlatformHeadlessRenderer>>>>,
    #[cfg(target_os = "windows")]
...
impl TestPlatform {
    pub fn new(executor: BackgroundExecutor, foreground_executor: ForegroundExecutor) -> Rc<Self> {
        Self::new_with_options(executor, foreground_executor, Arc::new(NoopTextSystem), None)
            .expect("failed to build the test platform")
    }

    /// Builds a platform that really renders: a real text system, and a headless
    /// renderer for every window opened on it.
    pub(crate) fn new_with_headless_renderer(
        ...
        Self::new_with_options(
            ...
            text_system,
            Some(Box::new(move || {
                new_headless_renderer(allow_software_adapter)
            })),
        )
    }
    ...
    fn new_with_options(
        ...
        headless_renderer_factory: Option<
            Box<dyn Fn() -> anyhow::Result<Box<dyn PlatformHeadlessRenderer>>>,
        >,
    ) -> anyhow::Result<Rc<Self>> {
        #[cfg(target_os = "windows")]
        ...
        Ok(Rc::new_cyclic(|weak| TestPlatform {
...
impl Platform for TestPlatform {
    ...
    fn open_window(
        ...
        let renderer = match &self.headless_renderer_factory {
            Some(factory) => Some(factory()?),
            None => None,
        };
        // The window resolves sprites through the renderer's atlas, so it must
        // not fall back to the fake one.
        let sprite_atlas = renderer
            .as_ref()
            .map(|renderer| renderer.sprite_atlas())
            .unwrap_or_else(|| Arc::new(TestAtlas::new()));
